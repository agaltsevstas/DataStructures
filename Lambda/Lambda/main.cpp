...
int main()
{
    ...
    /// C++11
    {
        ...
        auto lambda2 = [&] // НЕ РЕКОМЕНДУЕТСЯ: Захват всех локальных переменных по ссылке, изменять - можно
        {
            a = 1; // Передача локальной переменной по ссылке, изменять можно
            b = 2; // Передача локальной переменной по ссылке, изменять можно
            ++ptr; // Передача локального указателя, изменять можно адрес
        ...
        auto lambda3 = [=] // НЕ РЕКОМЕНДУЕТСЯ: Захват всех локальных переменных по значению (копирование), изменять - нельзя, глобальные/статические изменять - можно
        {
            ...
            // ++ptr; // Передача локального указателя, изменять нельзя адрес
        ...
        auto lambda6 = [=]() mutable // Захват всех локальных переменных по значению (копирование), изменения для локальных переменных при выходе из функции - не сохраняться, для глобальных/статических - сохраняется
        {
            ...
            ++ptr; // Передача локального указателя, изменять можно адрес, изменения сохранятся
